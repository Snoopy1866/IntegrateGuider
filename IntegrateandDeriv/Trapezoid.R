Trapezoid<-function(f,a,b,eps=1e-5)
{ 
    if (a > b)
    {
        list <- Trapezoid(f,b,a,eps)
        n <- list$区间个数
        result <- - list$积分结果
    ...
    {
        p<-DerivMaxValue(f,a,b,2)#高阶导数最大值
        if (p == 0)
        {
            n <- 1
...
#复合辛普森公式
Simpson<-function(f,a,b,eps=1e-5)
{
    if (a > b)
    {
        list <- Simpson(f,b,a,eps)
        n <- list$区间个数
        result <- - list$积分结果
        return(list(区间个数 = n,积分结果 = result))
    }
    ...
    {
        q<-DerivMaxValue(f,a,b,2)#高阶导数最大值
        if (q == 0)
        {
            n <- 1
...
#---------------------------------------------
DerivMaxValue <- function(f,c,d,m)
{
  #求函数F的m阶导的最大值
  #F -- function类型
  #c,d -- 区间端点
